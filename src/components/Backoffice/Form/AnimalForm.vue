...
const handleSubmit = async () => {
  ...
  if (token) {
    if (isEdit.value) {
      await api.patch(`/animals/${form.id}`, form, token).then((res) => {
        editableAnimal.value = {...res.data};
        animals.value = animals.value.map((animal) => {
          if (animal.id === res.data.id) {
            return res.data;
          }
          return animal;
        });
...
watch(() => {
  if (isEdit.value) {
    form.id = editableAnimal.value.id;
    ...
    form.species = editableAnimal.value.species['@id'];
    form.race = editableAnimal.value.race['@id']
